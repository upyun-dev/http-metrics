...
function request(options, callback) {
  ...
  req.setConnectTimeout = function(newConnectTimeout) {
    let connectTimeout = +(newConnectTimeout || 50) * 1000;
  ...
  req.setTimeout = function(newTimeout) {
    let timeout = +(newTimeout || 80) * 1000;
    req.clearTimeout();
    ctid = setTimeout(function() {
      req.destroy();
    }, timeout);
  ...
  req.setConnectTimeout(options.connectTimeout);
  ...
  req.on('error', function(err) {
    ...
    if (err.code === 'ECONNRESET') {
      return callback(new Error('request timeout'), result);
    }
